...
app.get('/getData', function (req,res) {
     pool.connect(function(err,client,done) {
        ...
        // note that query needs to be a single string with no line breaks so built it up bit by bit
        	var querystring = " SELECT 'FeatureCollection' As type, array_to_json(array_agg(f)) As features  FROM ";
        	querystring = querystring + "(SELECT 'Feature' As type     , ST_AsGeoJSON(lg.geom)::json As geometry, ";
        	querystring = querystring + "row_to_json((SELECT l FROM (SELECT question, answer_1,answer_2,answer_3,answer_4,correct_answer) As l  )) As properties";
        	querystring = querystring + "   FROM formdata As lg limit 100 ) As f ";
        	console.log(querystring);
...
app.post('/uploadData',function(req,res){
	...
	pool.connect(function(err,client,done) {
		...
		var querystring = "INSERT into formdata (question,answer_1,answer_2, answer_3, answer_4, correct_answer, geom) values ('";
		querystring = querystring + req.body.question + "','" + req.body.answer_1 + "','";
